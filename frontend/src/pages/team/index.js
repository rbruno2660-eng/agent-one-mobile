        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
              {['Nome', 'E-mail', 'Função', 'Status'].map(h => (
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-xs" style={{ color: 'var(--muted)' }}>Carregando...</td></tr>
            )}
            {!loading && users.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-xs" style={{ color: 'var(--muted)' }}>Nenhum membro ainda</td></tr>
            )}
            {users.map(u => (
